...
Отдельно через тот же `/api/*` proxy проверены вход сотрудника, восстановление
сессии и собственный профиль (HTTP 200), запрет HR-сводки и чужого профиля
(HTTP 403), выход (HTTP 200). Это проверка API; полный браузерный сценарий
сотрудника с завершением активности в текущей работе не пройден.
