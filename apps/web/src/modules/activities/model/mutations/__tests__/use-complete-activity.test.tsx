...
import { useSessionExpiry } from "@/modules/auth";
...
describe("useCompleteActivity", () => {
  ...
  function renderMutation() {
    return renderHook(
      () => {
        useSessionExpiry();
        return useCompleteActivity(employeeId);
      },
      {
  ...
  it("invalidates all employee recommendation variants and the plan only for that employee", async () => {
    const affected = [
  ...
  it.each([401, 403])(
    "passes completion errors to the central session handler for status %s",
    async (status) => {
      const before = createProfile();
      const sessionKey = ["session", "current"];
      const session = { employee_id: employeeId, role: "employee" };
      queryClient.setQueryData(queryKey, before);
      queryClient.setQueryData(sessionKey, session);
      vi.mocked(activitiesApi.completeActivity).mockRejectedValueOnce(
        new ApiError("Completion unavailable", status),
      );
      const { result } = renderMutation();
      ...
      expect(queryClient.getQueryData(sessionKey)).toEqual(
        status === 401 ? null : session,
      );
      expect(queryClient.getQueryData(queryKey)).toEqual(
        status === 401 ? undefined : before,
      );
      expect(result.current.completion).toBeUndefined();
      expect(activitiesApi.completeActivity).toHaveBeenCalledTimes(1);
    },
