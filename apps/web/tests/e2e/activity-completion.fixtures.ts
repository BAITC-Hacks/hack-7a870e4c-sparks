...
export async function mockCompletionApi(
  ...
  await page.route("**/api/**", async (route) => {
    ...
    if (path === `/api/employees/${employeeId}/plan`) {
      expect(request.method()).toBe("GET");
      const plan: GetCareerPlan200 = {
        employee_id: employeeId,
        current_position: { role: "Аналитик", grade: "Middle" },
        career_goal: target,
        goal_source: "explicit",
        as_of_date: catalog.as_of_date,
        last_review_date: state.profile.employee.last_review_date,
        effective_skills: state.profile.effective_skills,
        counted_completions: [],
        assessment_note: "Проверка пустого плана по синтетическому профилю.",
        readiness: state.profile.readiness,
        projected_readiness: state.profile.readiness,
        projected_skills: state.profile.effective_skills,
        trajectory: [],
        gaps: state.profile.gaps,
        recommendations: [],
        uncovered_skills: [],
        mandatory_tasks: [],
        warnings: [],
