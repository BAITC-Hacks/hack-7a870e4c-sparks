...
Стадии 5/6 реализованы: HR-сводка, каталог сотрудников с просмотром профиля и
импорт подключены к API. Для стадии 7 подготовлен и проверен локальный запуск
полного приложения одной командой. Результаты текущих проверок отделены ниже
от исторических тестов стадий 3/4; в текущей работе тесты не создавались и не
запускались по указанию пользователя.
...
**Статус на 23.09.2026: реализована и объединена со стадией 4.** Dashboard,
детали активности, карьерный план и советник подключены через module API adapters
и ручные TanStack Query hooks. До объединения проверены TypeScript, production
build, dependency boundaries, 108 unit/component tests и 15 Chromium-сценариев
с HTTP-ответами по контракту.
Живой AI и сквозной сценарий с настоящим API этой проверкой не подтверждены.
Команды и ограничения: [отчёт стадии 3](./stage-3-verification.md).

Детали восстанавливаются из рекомендаций. Если шаг отсутствует в подборе и
завершение не сохранено, используется `/plan`. Для завершённого шага прямая
ссылка показывает запись истории. Отдельного endpoint мероприятия нет; если
шага нет в рекомендациях, плане и истории, показывается состояние недоступности.
Демонстрационное подтверждение прохождения добавлено стадией 4.
...
Реализованы `modules/activities`, маршрут `/employee/activity/[activityId]`
и следующий шаг на `/employee`. `EmployeeDashboard` сохраняет
`RecommendationsSection` стадии 3. Единый `ActivityDetails` использует общие
`RecommendationContent` и `RecommendationMode` для объяснений и режима подбора,
а также завершение и историю стадии 4. После завершения обновляется профиль и
инвалидируются рекомендации, карьерный план и данные советника.
...
- Отдельного endpoint деталей мероприятия нет. Прямая ссылка загружает текущие
  рекомендации. Если шага в них нет и завершение ещё не сохранено, загружается
  карьерный план. После завершения показывается сохранённая история, если
  мероприятие уже исключено из подбора. Если шага нет в рекомендациях, плане и
  истории — состояние недоступности без кнопки завершения. Before/after доступно
  в текущем успешном запуске; после reload старый снимок не восстанавливается
  и не выдумывается.
...
Проверки отдельной стадии 4 до объединения: unit-тесты adapter/hook/mapper и
route-mocked Playwright-сценарии в `tests/e2e/activity-completion.spec.ts`.
Они проверяют ответ сервера,
...
AI-подбор. Команды и ограничения проверок указаны в README frontend. Результаты
отдельных стадий не подтверждают повторную проверку объединённой версии.

После объединения прошли 150 unit/component tests, 54 Playwright-сценария
в Chromium desktop/mobile, TypeScript, dependency boundaries и production
build. Проверены переходы из плана и допустимый повтор `EV_036`; команды и
границы проверки — в [README frontend](../README.md).
...
**Статус:** реализована. Проверки типов, границ зависимостей, сборки и Biome
пройдены; перечисленные ниже состояния UI не означают, что каждое из них
вручную проверено на живом сервере.
...
- `modules/hr-overview` содержит API-адаптер и запросы сводки/списка сотрудников;
  `modules/employees-directory` отвечает за каталог и одну боковую панель.
  Страницы `/hr` и `/hr/employees` остаются композиционными маршрутами.
- На сводке показаны `total_employees`, число сотрудников без следующего шага,
  дефициты навыков, причины от backend и агрегаты участия по мероприятиям.
  `completed`, `in_progress`, `other`, `total` считаются записями истории,
  включая ежегодные повторы, а не уникальными людьми. Обязательные мероприятия
  помечены отдельно; публичного рейтинга сотрудников нет.
- Поиск и фильтры отдела, роли и грейда обрабатывают полученный HR-список.
  Параметры `q`, `department`, `role`, `grade` сохраняются в URL через nuqs.
  Серверная фильтрация/пагинация не добавлялась.
- `employee` в URL управляет единственным Sheet. Открытие добавляет запись
  истории браузера, обычное закрытие заменяет ее. Профиль загружается через
  публичный API модуля `career-profile`: цель, навыки, разрывы, предупреждения
  и история участия. Покрытие требований цели не объявляется вероятностью
  повышения, отсутствие readiness не заменяется нулем.
- HR-запросы включаются только при HR-сессии; `meta.requiresSession` подключает
  обработку истечения сессии. Для загрузки, пустых данных, ошибки с повтором,
  недоступного профиля и отказа в доступе предусмотрены состояния UI на RU/KK.
  Backend остается источником проверки прав.
- HR-запросы разрешают повторную загрузку устаревших данных при возвращении на
  экран. Изменение цели и завершение активности инвалидируют HR-сводку.
...
**Статус:** реализована. Полная доменная валидация и атомарное сохранение
остаются на стороне API; frontend не имитирует успешный импорт.
...
Один из файлов может быть пустым. Frontend ограничивает суммарное содержимое
и сериализованный JSON-запрос 8 МиБ; backend ограничивает суммарное содержимое
двух строк 8 МиБ. Backend сначала
...
- `modules/data-import` содержит адаптер generated-клиента, чтение файлов,
  структурную проверку, предпросмотр и mutation. Формат отправки — JSON
  `{ employees_json, history_csv }`, без multipart.
- Доступны выбор/перетаскивание `.json` и `.csv`, замена и удаление файлов,
  количество записей и до трех примеров. Устаревшее чтение после замены файла
  не подменяет новый выбор. Требуется корректный UTF-8.
- JSON должен быть объектом с массивом `employees`; локально проверяются
  базовая форма, наличие и дубликаты ID. CSV проверяется с учетом запятых и
  переносов внутри кавычек, BOM, обязательных 10 колонок, необязательной
  `completed_at`, уникального заголовка и длины строк. JSON с BOM отклоняется
  по текущим правилам API. Значения полей, ссылки и дату среза проверяет сервер.
- Слишком большой отдельный файл не читается. До отправки вычисляется размер
  UTF-8 содержимого и JSON-запроса, включая экранирование. Пустое содержимое,
  ошибки чтения/структуры и превышение лимита блокируют отправку.
- Запрос выполняется только с HR-сессией, без автоматических повторов.
  UI показывает состояние отправки и итоговые `employees`, `history`,
  `message`; количества обозначают принятые строки, включая обновления
  существующих ID. После успеха доступны переход в HR и новый импорт.
- `400` показывает сообщение API, `413` — лимит; при сетевом сбое или `5xx`
  UI не утверждает, что сохранения не было, и предлагает проверить данные.
  Импорт не выдает учетные записи и не меняет роли.
- После успеха отменяются незавершенные чтения HR, каталога, профилей,
  рекомендаций, планов и советника. Неактивные снимки удаляются, чтобы
  глобальное `refetchOnMount:false` не оставило устаревший профиль при возврате;
  активные запросы инвалидируются. Успех не ждет медленного AI-обновления.
  Проверка личности сессии и mutation защищает кеш от позднего ответа после
  выхода или замены сессии. Частичные оптимистичные изменения не применяются.
...
### Реализованный запуск и фактические проверки

`./init.sh` теперь поднимает frontend вместе с API, PostgreSQL и Python-агентом
и открывает `/ru/login`. По умолчанию приложение доступно на порту 3000,
Swagger/API — 3001; `WEB_PORT` и `API_PORT` позволяют изменить внешние порты.
Вариант `./init.sh --no-open` пропускает открытие браузера.

Next.js проксирует `/api/*` к серверному `API_INTERNAL_URL` (локально
`http://127.0.0.1:3001`, при Docker-сборке `http://api:3001`). После изменения
этого адреса production необходимо пересобрать. Прямой
`NEXT_PUBLIC_API_URL` остается опциональным вариантом с настройкой CORS и
cookies. Примеры переменных исправлены; Nuqs adapter подключен в providers.

Полный запуск `./init.sh --no-open` выполнен в отдельном Compose-проекте
`career-stage7-check` с новой БД, frontend на порту 3310 и API на 3311.
Все четыре контейнера достигли healthy. Проверка шла без ключа OpenAI.

В production-интерфейсе на порту 3310 вручную проверены вход HR, поиск `E0001`
(1 из 200 с сохранением `q` в URL) и профиль в Sheet. Back закрывает панель и
возвращает фокус; Forward и reload восстанавливают выбранный профиль. Импорт
`examples/jury/employees.json` и `activity_history.csv` через UI показал
предпросмотр 2/6 и получил от API успешный результат 2 профиля / 6 строк.
Проверка не охватывает все пустые и ошибочные состояния.
...
Через тот же `/api/*` proxy отдельно проверены вход сотрудника, сессия и
собственный профиль (HTTP 200), HR-сводка и чужой профиль (HTTP 403), выход
(HTTP 200). Полный браузерный сценарий сотрудника с завершением активности
в текущей работе не пройден.

В текущей работе прошли `pnpm orval` (без изменений generated-клиента),
`pnpm exec tsc --noEmit --incremental false`, `pnpm lint:deps` (230 модулей),
`pnpm build` и Biome 33 измененных TS/TSX/JSON-файлов. Тесты не создавались и не запускались
по указанию пользователя. Исторические 150 unit/component tests и
54 Playwright-сценария выше относятся к объединению стадий 3/4 и не являются
проверкой стадий 5/6. Локальный запуск не подтверждает публичное развертывание,
живой OpenAI или прохождение всех состояний и ошибочных сценариев UI.
...
### Справочный перечень проверок

Это перечень для полной проверки интеграции, а не отчет о командах текущей
работы. Фактически выполненные проверки перечислены выше.
...
- [x] `generated.ts` сгенерирован из `apps/api/openapi.json`, Petstore удалён.
- [x] Прокси `/api/*`, опциональный `NEXT_PUBLIC_API_URL`, cookie credentials и `401` подключены.
...
- [x] Рекомендации показывают режим, до трёх результатов и объяснение по трём факторам.
- [x] Career plan и AI-advisor проверены отдельно, включая `503` (контрактные HTTP-моки).
- [x] Завершение активности идемпотентно и обновляет server-state из ответа.
- [x] HR overview, employee directory и profile Sheet реализованы, состояния пустых массивов предусмотрены.
- [x] Импорт структурно валидируется, соблюдает лимит 8 МиБ и отправляется атомарным запросом.
- [x] Полное приложение запускается через `./init.sh`; запуск проверен в отдельном Compose-проекте.
