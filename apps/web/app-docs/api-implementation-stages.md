...
- `/hr` подключён к `GET /api/hr/overview` через `modules/hr-overview/api`
  и `useHrOverview`. Показаны `total_employees`, сотрудники без следующего
  шага с исходным `reason`, дефициты навыков и участие по мероприятиям.
- Текущий API отдаёт для участия **числа записей**, а не проценты или число
  уникальных участников: `completed`, `in_progress`, `other`, `total`.
  Обязательные мероприятия помечаются отдельно. API не разделяет `other`
  на пропуски/отказы и не помечает HR-дефициты как critical: эти показатели
  из первоначальных UI-макетов не выдумываются. Публичных рейтингов нет.
- `/hr/employees` использует `useHrEmployees` и тот же API-adapter для
  `GET /api/employees`. Scenario-модуль `employees-directory` показывает
  таблицу на desktop и список на mobile. Поиск, подразделение, роль, грейд
  и фильтр «Без следующего шага» применяются к полученному списку; последний
  использует IDs из HR overview и показывает исходные причины.
- В layout подключён `NuqsAdapter`. URL хранит `q`, `department`, `role`,
  `grade`, `withoutStep` и выбранный `employee`. Фильтры меняются через
  replace, открытие единственного управляемого Sheet — push, обычное
  закрытие — replace. Работают Back/Forward, reload и прямые ссылки.
- Sheet переиспользует `useEmployeeProfile` и `mapProfileToView` через
  read-only `HrEmployeeProfile`: цель и её источник, readiness, навыки,
  warnings и полная история с обязательными повторениями. `null` readiness
  показан тире; локального пересчёта нет. Смена ID не показывает профиль
  предыдущего сотрудника. Повторное открытие обновляет профиль из API;
  HR-сводка и список также обновляются при возврате на страницу.
- HR-компоненты загружают данные только для HR-сессии, маршруты защищены
  `RolePage`. `401` очищает сессию и ведёт на вход, `403` показывает запрет,
  `404` профиля — отсутствие сотрудника. На эти ошибки нет автоматических
  повторных запросов профиля. Empty/loading/error/retry поддерживаются
  отдельно от успешных нулевых показателей. Интерфейс доступен на RU/KK.

Проверка: прошли 131 unit-тест (включая HR queries, фильтры, profile mapper
и обновление профиля при повторном открытии), 38 Playwright-сценариев стадий
4–5 с типизированными ответами API на desktop/mobile, TypeScript,
dependency boundaries и production-сборка. Отдельно на живом локальном API
пройден HR-путь: вход → сводка → список из 200 сотрудников → открытие
и закрытие профиля, без изменения данных. Общие lint-проверки сохраняют
ошибки исходного шаблона; Biome изменённых файлов проходит.
Публичное развёртывание и полный сценарий с живым AI не проверены.
Импорт остаётся стадией 6.
...
- [x] HR overview, employee directory и profile Sheet работают с пустыми массивами.
