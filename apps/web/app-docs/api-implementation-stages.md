...
**Статус на 23.09.2026: реализована.** Dashboard, детали активности, карьерный
план и советник подключены через module API adapters и ручные TanStack Query
hooks. Проверены TypeScript, production build, dependency boundaries, 108
unit/component tests и 15 Chromium-сценариев с HTTP-ответами по контракту.
...
Детали восстанавливаются из рекомендаций; если выбранный шаг есть только в
карьерном плане, используется `/plan`. Отдельного endpoint мероприятия нет.
Если шаг отсутствует в обоих ответах, показывается явное состояние «не найден».
Подтверждение прохождения остаётся задачей стадии 4; фиктивного запуска нет.
...
- [ ] Завершение активности идемпотентно и обновляет server-state из ответа.
