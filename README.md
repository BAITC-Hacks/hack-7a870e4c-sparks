...
- Демонстрационный сотрудник: логин `E0005`; HR: логин `hr`. Пароли опубликованной версии передаются жюри отдельно. При локальном запуске используются `DEMO_EMPLOYEE_PASSWORD` и `HR_PASSWORD` из своего `.env`.
...
Действующие пароли, API-ключи и `CREDENTIAL_SECRET` не публикуются в README или Git. `CREDENTIAL_SECRET` — серверный секрет начальных индивидуальных паролей, а не данные входа для жюри.
